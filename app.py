...
# 1) Weights block detection is now robust:
#    - Locates "SUM" row in Graphing!H and reads from the next row down
#    - Prevents skipping Cash / early tickers (root cause of META 7.xx% inflation)
# 2) Weight parsing is robust:
#    - Handles 0.0635 vs 6.35 vs "6.35%" consistently
# 3) Weights are only normalized for rounding noise (not to compensate for missing rows)
# 4) Sector chart fallback on Cloud:
#    - If sector fetch fails -> "Other"
# 5) White readable UI + top padding (Cloud header cut-off mitigation)
# 6) Logo sizing is controlled by two variables:
#    - SIDEBAR_LOGO_WIDTH and CSS_LOGO_MAX_HEIGHT
...
# --- Logo controls (edit these directly) ---
SIDEBAR_LOGO_WIDTH = 150   # <--- adjust here (e.g., 120–180)
CSS_LOGO_MAX_HEIGHT = 120  # <--- adjust here (e.g., 90–150)
...
st.markdown(
    ...
    <style>
      ...
      /* Global image cap */
      img {{
        max-height: {CSS_LOGO_MAX_HEIGHT}px !important;
...
def show_logo_sidebar() -> bool:
    for f in LOGO_FILES:
        try:
            st.sidebar.image(f, width=SIDEBAR_LOGO_WIDTH)
            return True
        except Exception:
            continue
    return False
...
def find_block_start_after_sum(grid: pd.DataFrame, label_col: int, fallback_row: int) -> int:
    """
    Robustly find the weights block start row in Graphing tab:
    - Look for a row where column label_col is 'SUM' (case-insensitive).
    - Start reading from the next row.
    """
    col = grid.iloc[:, label_col].apply(clean_str).str.strip()
...
def load_portfolio(portfolio_url: str) -> PortfolioData:
    ...
    g = read_public_sheet_range(pid, sheet="Graphing", cell_range="A1:P400")

    # ---- FIX: dynamically start weights block below the SUM row ----
    # Column H = index 7, Column I = index 8
    fallback_start = 7  # original "H8" assumption (0-index)
    weights_start = find_block_start_after_sum(g, label_col=7, fallback_row=fallback_start)

    w = extract_list_from_grid(g, row_start=weights_start, col_label=7, col_value=8)
    w.columns = ["Security", "Weight"]
    w["Security"] = w["Security"].astype(str).str.strip()

    w["Weight"] = w["Weight"].apply(to_num)

    # robust percent/fraction coercion
    def weight_to_frac(x):
        if pd.isna(x):
            return np.nan
        x = float(x)
        # percent-looking numbers (e.g., 6.35) -> 0.0635
        if 1.0 < x <= 100.0:
            return x / 100.0
        return x

    w["Weight"] = w["Weight"].apply(weight_to_frac)

    # clean
    w = w[w["Security"].str.len() > 0]
    w = w[w["Security"].str.lower().ne("nan")]
    w = w[~w["Security"].str.upper().isin(["SUM", "TOTAL"])]
    w = w.dropna(subset=["Weight"])
    w = w[w["Weight"] > 0]

    # IMPORTANT: only normalize for minor rounding (not to compensate for missing rows)
    # If your block is correct, sum should be ~1.0 already.
    wsum = float(w["Weight"].sum())
    if wsum > 0 and not (0.985 <= wsum <= 1.015):
        w["Weight"] = w["Weight"] / wsum

    # K/L current pnl
    r0 = 7  # still starts at row 8 for these blocks per template
...
def lookup_sectors(tickers: list[str]) -> dict[str, str]:
    """
    Cloud-safe:
    - Retries a few times (Streamlit Cloud IPs get throttled)
    - If sector missing -> "Other"
    """
    out = {}
    tickers = [t.upper().strip() for t in tickers if t and str(t).strip()]

    if yf is None:
        return {t: ("Cash" if t in {"CASH", "USD"} else "Other") for t in tickers}
...
def plot_donut(df: pd.DataFrame, label_col: str, value_col: str, title: str, top_n=8):
    ...
    with st.expander("Show full breakdown"):
        ...
        # show as % to match the Google Sheet
        d_full["Weight"] = (d_full[value_col] * 100).round(2).astype(str) + "%"
        st.dataframe(d_full[[label_col, "Weight"]], use_container_width=True, hide_index=True)
...
with cA:
    ...
    if len(pdata.weights) == 0:
        st.error("Weights not found in Graphing tab.")
    else:
...
with cB:
    ...
    if len(tickers) == 0:
        ...
        rows = []
        for t in tickers:
            sec = sectors.get(t, "Other") or "Other"
...
with st.expander("Debug", expanded=False):
    st.write("Weights start row (found below SUM):")
    st.write("If this is wrong, your Graphing tab has changed layout.")
    st.write("Weights sum (should be ~1.0):", float(pdata.weights["Weight"].sum()) if len(pdata.weights) else None)
    dbg = pdata.weights.copy()
    dbg["Weight_%"] = (dbg["Weight"] * 100).round(2)
